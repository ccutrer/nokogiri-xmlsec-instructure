...
CrossRuby = Struct.new(:version, :platform) do
  ...
  def dll_staging_path
    "tmp/#{platform}/stage/lib/#{NOKOGIRI_XMLSEC_SPEC.name}/#{minor_ver}/#{NOKOGIRI_XMLSEC_SPEC.name}.#{dll_ext}"
  end
...
def add_file_to_gem(relative_source_path)
  ...
  NOKOGIRI_XMLSEC_SPEC.files << relative_source_path
end

def gem_build_path
  File.join("pkg", NOKOGIRI_XMLSEC_SPEC.full_name)
end
...
def verify_dll(dll, cross_ruby)
  ...
  if cross_ruby.windows?
    ...
    unless /file format #{Regexp.quote(cross_ruby.target_file_format)}\s/.match?(dump)
      raise "unexpected file format for generated dll #{dll}"
    end
...
namespace "gem" do
  CROSS_RUBIES.find_all { |cr| cr.windows? || cr.linux? || cr.darwin? }.map(&:platform).uniq.each do |plat|
    ...
    task plat do
      RakeCompilerDock.sh(<<~TEXT, platform: plat, verbose: true)
        ruby -v &&
    ...
    namespace plat do
      ...
      task "builder" do
        ...
        Rake::Task["pkg/#{NOKOGIRI_XMLSEC_SPEC.full_name}-#{Gem::Platform.new(plat)}.gem"].invoke
      end
    end
  end
...
Rake::ExtensionTask.new("nokogiri_xmlsec", NOKOGIRI_XMLSEC_SPEC.dup) do |ext|
  ext.source_pattern = "*.{c,cc,cpp,h}"

  ext.lib_dir = File.join(*["lib", "nokogiri", "xmlsec", ENV["FAT_DIR"]].compact)
  ...
  ext.cross_compiling do |spec|
    ...
    headers_dir = "ext/nokogiri_xmlsec/include"

    ["xmlsec"].each do |lib|
      raise "#{lib} headers are not present in #{headers_dir}" unless File.directory?(File.join(headers_dir, lib))
    end
